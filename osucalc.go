...
func CalcGradeSTD(hit300 int, hit100 int, hit50 int, miss int) *accuracygrade {
	...
	equationshort := float64(((hit300 * 300) + (hit100 * 100) + (hit50 * 50)) / (300 * (hit300 + hit100 + hit50 + miss)))
	...
	if float32(float32(hit300)/totalhits) > 0.8 && miss == 0 || float32(float32(hit300)/totalhits) > 0.9 {
		grades.Grade = "A"
	}
	if float32(float32(hit300)/totalhits) > 0.9 && miss == 0 && float32(float32(hit50)/totalhits) < 0.01 {
		grades.Grade = "S"
	}
	if float32(float32(hit300)/totalhits) == 1 {
		grades.Grade = "SS"
	}

	formattedacc, err := strconv.ParseFloat(strconv.FormatFloat(equationshort, 'f', -1, 32), 32)
	grades.Accuracy = formattedacc
...
func CalcGradeTaiko(hit300 int, hit100 int, miss int) *accuracygrade {
	...
	grades.Accuracy = math.Round(float64((float32(hit300) + (float32(hit100) / 2))/float32(hit300 + hit100 + miss))*100) / 100
...
func CalcGradeCatch(hit300 int, hit100 int, hit50 int, hitkatu int, miss int) *accuracygrade {
	...
	grades.Accuracy = math.Round(float64(tophalf/bottomhalf)*100) / 100
